...
namespace BOforUnity.Examples
{
    public class FittsLawTask : MonoBehaviour
    {
        ...
        [Tooltip("When enabled (baseline blocks only), each round replays a precomputed botorch-Sobol design instead of a fresh random one. " +
                 "The Sobol points are read by round index, so all three scale blocks (5/20/100) present the identical set of designs.")]
        public bool useBaselineSobolDesigns = false;
        [Tooltip("Relative to StreamingAssets/BOData. Unit-cube Sobol matrix generated by generate_baseline_sobol.py; " +
                 "one ';'-separated column per design-parameter key, one row per baseline round.")]
        public string baselineSobolDesignFileName = "baseline_sobol_unit.csv";
        ...
        private List<string> _baselineSobolKeys;
        private List<float[]> _baselineSobolRows;
        private bool _baselineSobolLoadAttempted;
        // UTF-8 without a BOM, so app-log CSVs stay byte-clean and BOM-free like the rest of the pipeline.
        ...
        /// <summary>
        /// Enables (or disables) deterministic botorch-Sobol baseline designs. When enabling,
        /// the precomputed Sobol matrix is loaded eagerly so a missing/invalid file surfaces
        /// immediately rather than silently falling back to random sampling mid-block.
        /// </summary>
        public void SetUseBaselineSobolDesigns(bool enabled)
        {
            useBaselineSobolDesigns = enabled;
            if (enabled)
                LoadBaselineSobolDesignsIfNeeded();
        }

        private bool TryApplyBaselineSobolDesignForCurrentRound()
        {
            if (!useBaselineSobolDesigns || !LoadBaselineSobolDesignsIfNeeded())
                return false;

            // The round index is 1-based and set via SetManualLogContext immediately before
            // BeginTask. Indexing the Sobol rows by round (not a running counter) guarantees
            // every scale block replays the identical design set and is restart-safe.
            int round = _manualLogContextActive ? _manualLogIteration : 1;
            return ApplyBaselineSobolDesign(round - 1);
        }

        private bool ApplyBaselineSobolDesign(int rowIndex)
        {
            if (_baselineSobolRows == null || _baselineSobolRows.Count == 0)
                return false;

            // Defensive wrap so a block with more rounds than rows never throws.
            int count = _baselineSobolRows.Count;
            rowIndex = ((rowIndex % count) + count) % count;
            float[] row = _baselineSobolRows[rowIndex];
            BoForUnityManager source = GetRuntimeDesignParameterSource();
            // Same contract as the random/BO paths: when a parameter list is configured, only
            // keys present in it are applied; missing keys stay at their serialized value.
            bool useConfiguredParameterList = HasConfiguredParameterList(source);
            bool buttonColorChanged = false;

            if (TryGetBaselineSobolValue(row, xFontSizeParameterKey, source, useConfiguredParameterList, out float xFontSize))
                xFontSizePixels = xFontSize;

            if (TryGetBaselineSobolValue(row, buttonSizeParameterKey, source, useConfiguredParameterList, out float buttonSize))
                circleSizePixels = buttonSize;

            if (TryGetBaselineSobolValue(row, buttonDistanceParameterKey, source, useConfiguredParameterList, out float buttonDistance))
                circleDistancePixels = buttonDistance;

            if (TryGetBaselineSobolValue(row, buttonHueParameterKey, source, useConfiguredParameterList, out float hue))
            {
                buttonHue = Mathf.Repeat(hue, 1f);
                buttonColorChanged = true;
            }

            if (TryGetBaselineSobolValue(row, buttonSaturationParameterKey, source, useConfiguredParameterList, out float saturation))
            {
                buttonSaturation = Mathf.Clamp01(saturation);
                buttonColorChanged = true;
            }
            ...
            // Same no-overlap clamp the BO/random design paths apply, keeping the realized
            // design valid (and deterministic) for the chosen button size.
            EnforceNoOverlapDistanceForCurrentDesign();
            SyncRuntimeDesignParameterSourceWithCurrentDesign();
            return true;
        }

        // Resolves the [0,1] Sobol value for a parameter key and denormalizes it to the live
        // parameter bounds (falling back to the task-configured range when no BO source exists).
        private bool TryGetBaselineSobolValue(
            float[] row,
            string key,
            BoForUnityManager source,
            bool useConfiguredParameterList,
            out float value)
        {
            value = 0f;
            if (row == null || _baselineSobolKeys == null || string.IsNullOrWhiteSpace(key))
                return false;

            int col = _baselineSobolKeys.IndexOf(key);
            if (col < 0 || col >= row.Length)
                return false;

            if (!TryGetBaselineParameterRange(source, key, useConfiguredParameterList, out float lo, out float hi))
                return false;

            value = Mathf.Lerp(lo, hi, Mathf.Clamp01(row[col]));
            return true;
        }

        private bool TryGetBaselineParameterRange(
            BoForUnityManager source,
            string key,
            bool useConfiguredParameterList,
            out float lo,
            out float hi)
        {
            lo = 0f;
            hi = 1f;
            if (TryFindBoParameter(source, key, out ParameterArgs parameter))
            {
                lo = Mathf.Min(parameter.lowerBound, parameter.upperBound);
                hi = Mathf.Max(parameter.lowerBound, parameter.upperBound);
                return true;
            }

            // A configured parameter list intentionally fixes any key it omits.
            if (useConfiguredParameterList)
                return false;

            Vector2 range;
            if (key == xFontSizeParameterKey) range = xFontSizeRangePixels;
            else if (key == buttonSizeParameterKey) range = buttonSizeRangePixels;
            else if (key == buttonDistanceParameterKey) range = buttonDistanceRangePixels;
            else if (key == buttonHueParameterKey) range = buttonHueRange;
            else if (key == buttonSaturationParameterKey) range = buttonSaturationRange;
            else return false;

            lo = Mathf.Min(range.x, range.y);
            hi = Mathf.Max(range.x, range.y);
            return true;
        }

        // Loads the unit-cube Sobol matrix (header = parameter keys, rows in [0,1]) once.
        private bool LoadBaselineSobolDesignsIfNeeded()
        {
            if (_baselineSobolLoadAttempted)
                return _baselineSobolRows != null && _baselineSobolRows.Count > 0;

            _baselineSobolLoadAttempted = true;
            _baselineSobolKeys = new List<string>();
            _baselineSobolRows = new List<float[]>();

            try
            {
                string path = Path.Combine(Application.streamingAssetsPath, "BOData", baselineSobolDesignFileName);
                if (!File.Exists(path))
                {
                    Debug.LogError(
                        $"FittsLawTask: baseline Sobol design file not found at '{path}'. " +
                        "Run generate_baseline_sobol.py to create it. Falling back to random sampling.");
                    return false;
                }
                ...
                foreach (string rawLine in File.ReadAllLines(path))
                {
                    ...
                    if (!headerParsed)
                    {
                        for (int i = 0; i < cells.Length; i++)
                            _baselineSobolKeys.Add(cells[i].Trim());
                        headerParsed = true;
                        continue;
                    }

                    float[] values = new float[cells.Length];
                    bool ok = true;
                    for (int i = 0; i < cells.Length; i++)
                    {
                        if (!float.TryParse(cells[i].Trim(), NumberStyles.Float, CultureInfo.InvariantCulture, out values[i]))
                        {
                            ok = false;
                            break;
                        }
                    ...
                    if (ok)
                        _baselineSobolRows.Add(values);
                }

                if (!headerParsed || _baselineSobolRows.Count == 0)
                {
                    Debug.LogError($"FittsLawTask: baseline Sobol design file '{path}' contained no usable rows.");
                    return false;
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"FittsLawTask: failed to load baseline Sobol designs: {ex.Message}");
                return false;
            }
